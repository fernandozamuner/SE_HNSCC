...
TF.cistrome.p.10k <- ParseTumorCounts(counts.df, "Promoter", "10k", 5)
TF.cistrome.p.100k <- ParseTumorCounts(counts.df, "Promoter", "100k", 5)
TF.cistrome.e.10k <- ParseTumorCounts(counts.df,  "Enhancer", "10k", 5)
TF.cistrome.e.100k <- ParseTumorCounts(counts.df, "Enhancer", "100k", 5)
TF.cistrome.se.10k <- ParseTumorCounts(counts.df, "SE", "10k", 5)
TF.cistrome.se.100k <- ParseTumorCounts(counts.df, "SE", "100k", 5)
...
# plot promoters
p.rows <- plt.df$analysis == "Promoter Domains"
plt.p <- ggplot(plt.df[p.rows,],
            aes(x = normalOR, y = tumorOR)) +
  geom_point(col =pltcols[p.rows]) +
  xlab("Normal TF enrichment") +
  ylab("Tumor TF enrichment") +
  ggtitle("Promoters") +
  theme_classic() +
...
e.rows <- plt.df$analysis == "Typical Enhancer Domains"
...
se.rows <- plt.df$analysis == "Super Enhancer Domains"
...
# now plot the figure
# repeat the plot made for d but for TFs significantly
...
print(col_map)
print(plt.df.delta$cmap)
print(col_map[levels(plt.df.delta$cmap)])
...
plt.d <- ggplot(plt.df.delta, # plt.dn == delta normal
  ...
  labs(x = "TFs",
       y = expression(paste(Delta, "log(enrichment)")),
       color = "Legend") +
...
pdf("TFs_enrichments.pdf", width = 9, height = 6,
    onefile = F)
...
pdf("TFs_enrichments.qc.pdf", onefile = F)
ggarrange(
  ggarrange(
    plt.hist.10k,
    plt.hist.100k,
